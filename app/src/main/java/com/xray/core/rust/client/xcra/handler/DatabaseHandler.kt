...
object DatabaseHandler {
    ...
    private const val ID_ROUTER = "ASSET"
    ...
    private const val KEY_ROUTER_RULES = "ROUTER_RULES"
    ...
    private val routerStorage by lazy { MMKV.mmkvWithID(ID_ROUTER, MMKV.MULTI_PROCESS_MODE) }
     ...
     */
    fun decodeAssetItems(): List<Pair<String, AssetItem>> {
        val assetUrlItems = mutableListOf<Pair<String, AssetItem>>()
        assetStorage.allKeys()?.forEach { key ->
            val json = assetStorage.decodeString(key)
            if (!json.isNullOrBlank()) {
                assetUrlItems.add(Pair(key, JsonUtil.fromJson(json, AssetItem::class.java)))
            }
        }
     ...
     */
    fun removeAsset(uuid: String) {
        assetStorage.remove(uuid)
    }
     ...
     */
    fun encodeAsset(uuid: String, assetItem: AssetItem): String {
        val key = uuid.ifBlank { Utils.getUuid() }
     ...
     */
    fun decodeAsset(uuid: String): AssetItem? {
        val json =
            assetStorage.decodeString(uuid) ?: return null
        return JsonUtil.fromJson(json, AssetItem::class.java)
    }
    ...
    /**
     * Decodes the routing rules.
     *
     * @return The list of routing rules.
     */
    fun decodeRoutingRules(): MutableList<RuleItem>? {
        val ruleset = routerStorage.decodeString(KEY_ROUTER_RULES)
        if (ruleset.isNullOrEmpty()) return null
        return try {
            JsonUtil.fromJson(ruleset, Array<RuleItem>::class.java).toMutableList()
        } catch (_: Exception) {
            mutableListOf()
        }
    }
    ...
    /**
     * Encodes the routing rules.
     *
     * @param ruleList The list of routing rules.
     */
    fun encodeRoutingRules(ruleList: MutableList<RuleItem>?) {
        if (ruleList.isNullOrEmpty())
            routerStorage.encode(KEY_ROUTER_RULES, "")
        else
            routerStorage.encode(
                KEY_ROUTER_RULES,
                JsonUtil.toJson(ruleList)
            )
    }
    //endregion
}
