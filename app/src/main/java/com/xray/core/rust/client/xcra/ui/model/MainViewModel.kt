...
class MainViewModel(
    ...
    var scrollToTop: MutableState<Boolean> = mutableStateOf(false)
        private set
    ...
    fun swapNodes(from: LazyListItemInfo, to: LazyListItemInfo) {
        val fromPosition = nodeList.indexOf(nodes[from.index].uuid)
        val toPosition = nodeList.indexOf(nodes[to.index].uuid)
        val sortedItems = nodeList.toMutableList().apply {
            add(fromPosition, removeAt(toPosition))
        }
        DatabaseHandler.encodeNodeList(sortedItems)
        reloadNodes()
    }
