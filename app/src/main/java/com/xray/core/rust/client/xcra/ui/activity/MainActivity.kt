...
import com.xray.core.rust.client.xcra.ui.activity.GroupListActivity.Companion.GROUP_LIST_ACTIVITY_RESULT
...
class MainActivity : VpnActivity(), MainViewModel.MainViewModelInterface {
    private val startActivityForResult =
        registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
            if (result.resultCode == RESULT_OK) {
                if (result.data?.action == GROUP_LIST_ACTIVITY_RESULT) {
                    mainViewModel.reloadGroups()
                    mainViewModel.reloadNodes()
                }
                mainViewModel.updateScrollToTop(true)
            }
        }
    ...
    fun initAssets() {
        lifecycleScope.launch(Dispatchers.Default) {
            RouterHandler.initAssets(context = applicationContext, assets)
        }
    }
}
